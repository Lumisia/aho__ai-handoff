...
> ⚠️ **在 Claude Code 中，插件命令会按插件名加上命名空间。** 请输入 **`/ai-handoff:handoff`** —— 例如 `/ai-handoff:handoff status` —— 而不是裸的 `/handoff`（裸的 `/handoff` 会返回 *“Unknown command”*）。下表为便于阅读使用了简短的 `/handoff` 写法。在 **Codex** 中，交接相关动作来自内置技能，是 model-invoked 的 —— 直接用自然语言提出即可（例如 *“显示我的 ai-handoff 状态”*、*“恢复这次交接”*）。
