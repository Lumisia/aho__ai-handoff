...
Codex では既定で Stop hook からモデルを再実行して capsule を作りません。Codex の Stop `decision: "block"` と `reason` はユーザーに見える continuation prompt になるため、capsule 作成指示が会話に表示される可能性があります。

代わりに ai-handoff は `UserPromptSubmit` または `PostToolUse` で developer context を注入します。`auto` mode では Codex が最終回答の末尾に `ai-handoff-capsule` fenced JSON を付け、Stop hook はそれを parse して保存するだけです。`ask` mode では Stop 前に `request_user_input` で質問するよう指示します。threshold を Stop 時点で初めて検知した場合は、Codex を再実行せず静かに `DEGRADED_AVAILABLE` capsule を保存します。
...
| `codex.inline_final_capsule` | `true` | Codex auto mode で最終回答の fenced capsule flow を使うか |
| `codex.stop_continuation_auto_summary` | `false` | 旧 Codex Stop `decision:block` summary continuation を許可するか |
| `codex.stop_continuation_ask` | `false` | 旧 Codex Stop `decision:block` ask continuation を許可するか |
| `codex.degraded_fallback_on_stop` | `true` | Codex が Stop 時点で初めて threshold を超えた場合に degraded capsule を保存するか |
