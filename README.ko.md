...
> ⚠️ **Claude Code에서는 플러그인 명령이 플러그인 이름으로 네임스페이스됩니다.** bare `/handoff`가 아니라 **`/ai-handoff:handoff`** 로 입력하세요 — 예: `/ai-handoff:handoff status`. (bare `/handoff`는 *"Unknown command"* 가 뜹니다.) 아래 표는 읽기 편하게 짧은 `/handoff` 형태로 적었습니다. **Codex**에서는 handoff 동작이 번들 스킬에서 나오며 model-invoked입니다 — 그냥 말로 요청하세요 (예: *"내 ai-handoff 상태 보여줘"*, *"handoff 이어받아줘"*).
